...
\subsection{Enlarged sample}
The batch was then extended to every remaining SCHZ participant of ds000030 with a resting run and a T1w and to a further 20 controls. Of 22 SCHZ candidates 13 passed and 8 failed censoring (one was deferred after repeated container restarts); of 29 spare controls 3 lack a T1w on the OpenNeuro bucket and 3 failed censoring, and the first 20 that passed were kept, giving 40 controls and 33 with schizophrenia (df 71). The groups now differ in age ($29.4 \pm 7.9$ versus $35.9 \pm 9.2$ years, $p = 0.002$), motion ($0.094 \pm 0.036$ versus $0.125 \pm 0.040$\,mm, $p = 0.001$), censoring (4.5\% versus 9.6\%, $p = 0.002$) and residual amplitude ($0.374 \pm 0.051$ versus $0.319 \pm 0.076$, $p < 0.001$); no covariate is used, as in the pilot, since the comparison of interest is between builds on identical inputs. The per-participant picture is unchanged: the before-fix build returns NaN in 17.9\% of in-mask voxels (more in SCHZ, $23.1 \pm 15.6\%$, than in controls, $13.6 \pm 7.5\%$, $p = 0.001$), 0.30 of the correct value elsewhere, and maps correlating $r = 0.58$ with the fixed build. The uncorrected group contrast behaves as at $n = 40$: $t$-maps from the two builds correlate 0.63 (90\%-coverage mask) and 0.64 (intersection), change sign in 11--12\% of voxels, and their $p < 0.001$ sets share 15 of 293 and 7 of 104 voxels. The global-mean reduction of ReHo in schizophrenia is now significant under both builds ($t_{71} = -3.46$, $p = 0.001$, $d = -0.81$ before the fix with NaN as zero; $-3.60$, $p = 0.001$, $d = -0.85$ after), so the pilot's $p = 0.05$ crossing was a matter of sample size rather than of build.

Table~\ref{tab:clust73} gives the cluster-level result. With 73 participants a published design finds corrected clusters under both builds, all in the direction SCHZ $<$ CONTROL, but not the same ones. At the conventional cluster-forming $p < 0.001$ the two builds' corrected results share no voxel in either mask: the code every paper ran reports reduced ReHo in precuneus, posterior cingulate and right fusiform cortex, the fixed code in right middle temporal gyrus, or nothing in the intersection mask. At $p < 0.01$ the midline posterior-cingulate/precuneus cluster is found by both builds (146 of 610 voxels shared in the intersection mask), while the supplementary-motor and right-lingual clusters exist only before the fix and the cerebellar-vermis cluster only after it. This is the concrete form of the exposure: at the sample size and thresholds the affected studies use, each pipeline yields a publishable corrected finding, and the findings are anatomically different.

\begin{table}[t]
\centering\small
\caption{Cluster-level inference in the enlarged sample (33 SCHZ versus 40 CONTROL; \code{3dttest++ -Clustsim}, 10{,}000 sign-flip randomisations, NN1, bi-sided, $\alpha = 0.05$). All clusters are SCHZ $<$ CONTROL; labels are the Macro Labels atlas at the peak. Shared voxels: intersection / union of the two builds' corrected clusters.}
\label{tab:clust73}
\begin{tabular}{@{}llclp{5.6cm}c@{}}
\toprule
Mask & $p$ & Build & Threshold & Surviving clusters (voxels) & Shared \\
\midrule
90\%-coverage & 0.001 & before & 24 & 3 (91): precuneus 38, R fusiform 28, precuneus/mid-cingulate 25 & 0 / 120 \\
 & & after & 24 & 1 (29): R middle temporal & \\
 & 0.01 & before & 145 & 3 (1{,}289): precuneus 793, SMA 295, R lingual 201 & 102 / 1{,}719 \\
 & & after & 172 & 2 (532): cerebellar vermis 274, R middle temporal 258 & \\
Intersection & 0.001 & before & 14 & 3 (61): vermis/lingual 21, posterior cingulate 20, precuneus 20 & 0 / 61 \\
 & & after & 14 & none (largest 11) & \\
 & 0.01 & before & 79 & 3 (524): posterior cingulate 245, precuneus 150, vermis/lingual 129 & 146 / 610 \\
 & & after & 72 & 2 (232): posterior cingulate 151, precuneus 81 & \\
\bottomrule
\end{tabular}
\end{table}
