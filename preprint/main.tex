...
\label{sec:results}
Table~\ref{tab:merged} lists the sixteen pull requests sent upstream, the six the maintainers have accepted, and the ten under review; two further AFNI issues (Fisher-$z$ saturation in \code{3dcalc atanh}; the definition behind the ACF ``effective FWHM'') were filed without a patch. Behind them stand 36 confirmed findings in AFNI (9 judged capable of changing published numbers, 15 narrower, 12 likely) and 83 in SPM (5 high, 15 medium, 63 low priority), each with file-and-line evidence and a harness in the repository, alongside the code paths that were verified correct, which are recorded with equal care: in SPM the standard fMRI GLM was traced end to end (\code{spm\_spm}, the ReML machinery, the T and F Euler-characteristic densities, both FDR procedures, the NIfTI quaternion core) and held; in AFNI more than forty high-stakes paths held, most verified numerically.

\begin{table}[p]
\centering\small
\caption{Pull requests sent upstream from the AFNI and SPM reviews, as of 2 September 2026. Every correctness PR is paired with an issue carrying a runnable reproduction.}
\label{tab:merged}
\begin{tabular}{@{}llp{8.4cm}l@{}}
\toprule
Package & PR & Change & Status \\
\midrule
AFNI & 944 & \code{3dReHo}: detect rank ties on the float values, not integer-truncated copies; on percent-signal or standardised input almost every sample had been declared tied and Kendall's $W$ collapsed (Section~\ref{sec:reho}) & merged 28 Aug \\
AFNI & 947 & NIfTI reader: loop bound for sequential-descending slice timing compared against the wrong end, so every slice offset was zero and \code{3dTshift} silently did nothing, including on files AFNI itself writes & merged 28 Aug \\
AFNI & 926 & Test suite: \code{test\_3dttest++} never actually executed \code{3dttest++} & merged 28 Aug \\
AFNI & 928 & Test suite: return the exit code from asynchronous command execution, so failures are seen & merged 28 Aug \\
AFNI & 951 & \code{3dttest++}/\code{3dGroupInCorr}: \code{-paired -zskip} zeroes valid voxels; B$-$A negation slots; $t$-to-$z$ round-off saturation & open \\
AFNI & 953 & \code{3dTstat}: \code{-DW} denominator, \code{-tdiff} detrend convention, \code{-nzmean} NaN & open \\
AFNI & 956 & \code{3dBrickStat}: \code{-automask} truncates the voxel scan; \code{-absolute} uses integer \code{abs} & open \\
AFNI & 958 & \code{3dTshift -no\_detrend}: demeans the wrong array for the second voxel of each pair & open \\
AFNI & 960 & \code{3dMEMA}/\code{3dLMEr}/\code{3dMVM}: missing-data degrees-of-freedom bookkeeping; $p$-to-statistic conversions & open \\
AFNI & 962 & \code{3dROIstats -sigma}: Bessel factor computed in integer arithmetic & open \\
AFNI & 964 & \code{3dmaskave}/\code{mri\_percents}: one-past-end reads at extreme percentiles & open \\
AFNI & 966 & One-line fixes in \code{armacor}, \code{3dXClustSim}, \code{edt\_coerce}, \code{3dpc}, \code{3dDWItoDT} & open \\
\midrule
SPM & 163 & \code{@meeg/badsamples}: peristimulus baseline added a second time when mapping artefact events to samples, shifting every bad-sample window on epoched data by the baseline length & merged 2 Sep \\
SPM & 165 & \code{spm\_eeg\_downsample}: stamp the achieved, not the requested, sampling rate (1000$\to$180\,Hz actually yields 166.7\,Hz) & merged 2 Sep \\
SPM & 159 & \code{spm\_ECdensity}: $\chi^2$ Euler-characteristic densities need $t^{(\nu-d)/2}$, not one power for all orders; peak FWE far too conservative and cluster extent far too liberal on $\chi^2$ fields; ships with the function's first unit test & open \\
SPM & 161 & \code{spm\_nlsi\_GN}: free-energy log-determinant counted $n_q$ times too many after Kronecker expansion; affects DCM for evoked responses & open \\
\bottomrule
\end{tabular}
\end{table}

Two things about the merged set are worth stating. None was a suspicion: each pull request carried a reproduction the maintainer could run, and for the correctness fixes the maintainer's comment acknowledged the mechanism (for PR 944, that a mapping from values to ranks ``had not been added''; for PR 947, that descending sequences ``see minimal real-world usage'' despite affecting AFNI's own output). And none would have been found by starting from the literature: no methods section says ``we passed percent-signal data to a routine that truncates to integers.'' The review found the defect; the literature search, done afterwards, found who was exposed.

\subsection{Exposure, found after the fact}
For the \code{3dReHo} defect, 57 papers were adjudicated. Five are potentially affected: two normalised each voxel's series to unit variance, the worst case, and three ran \code{afni\_proc.py} with its default scaling. We do not identify them here. Whether a defect in the software changes a study's conclusions is a question only a rerun by its authors can answer, and the corresponding authors of the five studies have been contacted with the finding, the fix and the reanalysis below. Fifty-two are not, most of them cleared at once by the scaling conventions of the large wrapper pipelines (XCP-D keeps native units; HALFpipe, PhiPipe and CCS grand-mean scale to 10{,}000; C-PAC and DPABI do not call \code{3dReHo}). One paper that L2-normalised its input was first classified as exposed and moved out when the faithful port showed that regime to be exactly correct; that reversal is recorded because it is what verification is for. Eleven papers could not be read. For the SPM findings the exposure is anchored to methods papers rather than to individual studies: the artefact-window defect sits in the machinery documented by the SPM8 M/EEG reference paper, and the free-energy defect in DCM for evoked responses as introduced in its methods papers; a per-paper join for SPM, in the style of the ReHo pass, is future work.
