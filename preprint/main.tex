...
Large language models that can read and execute code make the search practical. This paper reports what happened when we pointed one at two of the most used neuroimaging packages, AFNI \cite{cox1996} and SPM, with a simple brief: read everything, assume nothing, and report only what you can reproduce. Two features of the design are deliberate. The review came first and the literature second, so that findings were not filtered by what we expected to matter. And nothing was reported to a maintainer without a numerical reproduction and, where possible, a patch, so that the unit of output is a pull request rather than a list of suspicions. Section~\ref{sec:results} reports what the maintainers merged. Section~\ref{sec:reho} then takes one merged fix and asks the question that matters to readers of the affected literature: how much did the results change?
...
\paragraph{Data and pipeline.} Resting-state fMRI and T1-weighted images from the UCLA Consortium for Neuropsychiatric Phenomics dataset, OpenNeuro ds000030 \cite{poldrack2016} (3\,T, TR 2\,s, 152 volumes). We attempted the first 20 eligible CONTROL and 20 SCHZ participants by identifier and drew from a spare pool until 20 of each passed \code{afni\_proc.py} (AFNI 26.2.06; blocks despike, tshift, align, tlrc, volreg, blur 4\,mm, mask, \textbf{scale}, regress; MNI152 2009 template; band-pass 0.01--0.1\,Hz; censoring at 0.3\,mm and 5\% outliers; motion parameters and derivatives as regressors). The default \code{scale} block converts the data to percent signal change, so the residual series handed to \code{3dReHo} have SD of order 0.3--0.5. Twelve of 52 attempted participants (4 of 24 CONTROL, 8 of 28 SCHZ) failed the pipeline's own regression step because censoring left fewer time points than the 108 nuisance regressors and were replaced; that exclusion is identical for both arms.

\paragraph{The two builds.} The fix landed in two commits on consecutive days: the tie-detection change itself (\code{94ee52b}, pull request 944) and a guard returning $W = 0$ when the tie correction exactly cancels the denominator (\code{74a90ac}). To isolate the tie-detection change we compare the source immediately before it (\code{4c2bd54}) with the guard applied against master at \code{d202535} (AFNI 26.2.06, bit-identical to the distributed build on test data), so the only difference is whether ties are detected on truncated integers or on floating-point values. \code{3dReHo -nneigh 27} from each build was run on the identical residuals and mask of every participant. A faithful Python port reproduces both builds voxel for voxel to four decimals on synthetic volumes. Releases before 26.2.04 (August 2026) did not close a tie run reaching the end of the sorted series, so in those releases a voxel whose entire series lies inside one integer bin is returned uncorrected, hence correct, rather than as zero; the truncation defect itself is common to every release.

\paragraph{Analysis.} Per participant: the ratio and relative error of before-fix to after-fix ReHo over all in-mask voxels, the fraction of voxels the before-fix build returns as exactly zero, and the voxelwise correlation of the two maps. Group level: \code{3dttest++} two-sample SCHZ versus CONTROL on each set of maps, in the intersection of all 40 masks (8{,}657 voxels) and in voxels present in at least 90\% of participants (25{,}227 voxels); the \code{3dttest++} output was reproduced to the last digit by an independent $t$-test.

\section{Results: what was found and what was merged}
\label{sec:results}
Table~\ref{tab:merged} lists the fixes the maintainers have accepted and those under review. Behind them stand 36 confirmed findings in AFNI (9 judged capable of changing published numbers, 15 narrower, 12 likely) and 83 in SPM (5 high, 15 medium, 63 low priority), each with file-and-line evidence and a harness in the repository, alongside the code paths that were verified correct, which are recorded with equal care: in SPM the standard fMRI GLM was traced end to end (\code{spm\_spm}, the ReML machinery, the T and F Euler-characteristic densities, both FDR procedures, the NIfTI quaternion core) and held; in AFNI more than forty high-stakes paths held, most verified numerically.

\begin{table}[t]
\centering\small
\caption{Fixes sent upstream from the AFNI and SPM reviews, as of 2 September 2026.}
\label{tab:merged}
\begin{tabular}{@{}llp{7.6cm}l@{}}
\toprule
Package & Change & Defect & Status \\
\midrule
AFNI & PR 944 & \code{3dReHo} detects rank ties on integer-truncated values; on percent-signal or standardised input almost every sample is declared tied and Kendall's $W$ collapses (Section~\ref{sec:reho}) & merged 28 Aug 2026 \\
AFNI & PR 947 & NIfTI reader compares against the wrong loop bound for sequential-descending slice timing, so every slice offset is zero and \code{3dTshift} silently does nothing, including on files AFNI itself writes & merged 28 Aug 2026 \\
SPM & PR 163 & \code{@meeg/badsamples} adds the peristimulus baseline a second time when mapping artefact events to samples, shifting every bad-sample window on epoched data by the baseline length; reaches automatic artefact rejection, averaging masks and robust covariance & merged 2 Sep 2026 \\
SPM & PR 165 & \code{spm\_eeg\_downsample} stamps the requested rather than the achieved sampling rate; for non-integer ratios every downstream time axis, latency and filter cutoff is silently off (1000$\to$180\,Hz actually yields 166.7\,Hz) & merged 2 Sep 2026 \\
SPM & PR 159 & \code{spm\_ECdensity} uses one power of $t$ for all $\chi^2$ Euler-characteristic orders where Worsley's densities need $t^{(\nu-d)/2}$; peak FWE on $\chi^2$ fields far too conservative, cluster extent far too liberal; ships with the function's first unit test & open \\
SPM & PR 161 & \code{spm\_nlsi\_GN} free energy counts the log-determinant term $n_q$ times too many after Kronecker expansion; affects DCM for evoked responses, where model ranking is the output & open \\
\bottomrule
\end{tabular}
\end{table}
