...
\paragraph{Data and pipeline.} Resting-state fMRI and T1-weighted images from the UCLA Consortium for Neuropsychiatric Phenomics dataset, OpenNeuro ds000030 \cite{poldrack2016} (3\,T, TR 2\,s, 152 volumes). I attempted all 50 SCHZ participants and the first 50 CONTROL participants by identifier that have a resting run, and kept every one that passed \code{afni\_proc.py} (AFNI 26.2.06; blocks despike, tshift, align, tlrc, volreg, blur 4\,mm, mask, \textbf{scale}, regress; MNI152 2009 template; band-pass 0.01--0.1\,Hz; censoring at 0.3\,mm and 5\% outliers; motion parameters and derivatives as regressors). The default \code{scale} block converts the data to percent signal change, so the residual series handed to \code{3dReHo} have SD of order 0.3--0.5. Three CONTROL participants have no T1-weighted image on the OpenNeuro bucket; 23 (7 CONTROL, 16 SCHZ) failed the pipeline's own regression step because censoring left fewer time points than the 108 nuisance regressors; one SCHZ participant could not be processed for infrastructure reasons. That leaves 40 CONTROL and 33 SCHZ (Table~\ref{tab:demo}); every exclusion is identical for both arms. The SCHZ group is older, moves more and loses more volumes, as clinical groups typically do.


\begin{table}[t]
\centering\small
\caption{The 73 participants. Motion is the mean Euclidean norm of the frame-to-frame motion derivative (mm), over all volumes and over the volumes retained after censoring. Group comparisons: Student $t$ (df 71) or Fisher's exact test.}
\label{tab:demo}
\begin{tabular}{@{}lccc@{}}
\toprule
 & CONTROL ($n = 40$) & SCHZ ($n = 33$) & $p$ \\
\midrule
Age, years (range) & $29.4 \pm 7.9$ (21--49) & $35.9 \pm 9.2$ (22--49) & 0.002 \\
Sex, F / M & 18 / 22 & 8 / 25 & 0.09 \\
Motion, all volumes & $0.094 \pm 0.036$ & $0.125 \pm 0.040$ & 0.001 \\
Motion, retained volumes & $0.086 \pm 0.029$ & $0.107 \pm 0.030$ & 0.004 \\
Volumes censored & 4.5\% & 9.6\% & 0.002 \\
Residual SD handed to \code{3dReHo} & $0.374 \pm 0.051$ & $0.319 \pm 0.076$ & $<0.001$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Analysis.} Per participant: the ratio and relative error of before-fix to after-fix ReHo over all in-mask voxels, the fraction of voxels the before-fix build returns as exactly zero, and the voxelwise correlation of the two maps. Group level: \code{3dttest++} two-sample SCHZ versus CONTROL on each set of maps, in the intersection of all 73 masks (6{,}488 voxels) and in voxels present in at least 90\% of participants (24{,}403 voxels); the \code{3dttest++} output was reproduced to the last digit by an independent $t$-test. Voxelwise maps were compared at uncorrected thresholds, since the question is how the two builds' maps differ rather than whether either is significant. Cluster-level inference, as a published design would report it, used \code{3dttest++ -Clustsim} (sign-flip randomisation of the residuals, 10{,}000 iterations, first-nearest-neighbour clustering, bi-sided) to obtain the cluster-size threshold at family-wise $\alpha = 0.05$ for cluster-forming $p = 0.001$ and $p = 0.01$ under each build and mask, followed by \code{3dClusterize}; surviving clusters were labelled with AFNI's Macro Labels atlas at the peak. No covariates were modelled: the question is how the two builds differ on identical inputs, and any covariate would enter both arms identically. The groups do differ in age and motion (Table~\ref{tab:demo}), so the clinical contrast itself is not adjusted, and no clinical claim is made from it.
...
Regional homogeneity \cite{zang2004} is Kendall's coefficient of concordance $W$ over the ranked time series of a voxel and its neighbours, with a correction $\sum_i (t_i^3 - t_i)$ over tie runs of length $t_i$ subtracted from the denominator. In AFNI's implementation \cite{taylor2013} (\code{src/ptaylor/rsfc.c}, lines 63--118) ranks came from a true floating-point sort, but tie \emph{detection} compared values stored into an integer array, so any two samples sharing an integer part were declared tied. For raw scanner units this almost never happens. For percent-signal-change residuals or z-scored series, nearly every sample lies in $(-1, 1)$ and truncates to zero: almost the whole series becomes one tie run, the correction approaches the denominator, and $W$ collapses toward zero, reaching exactly zero in voxels whose whole series lies in one integer bin. Figure~\ref{fig:mech} shows the behaviour of the faithful port against input scale with the 73 participants overlaid. The error is not a constant factor: it depends on the amplitude of each voxel's own series, so the before-fix statistic is partly a measure of BOLD variance; and the affected regime is exactly where \code{afni\_proc.py}'s default \code{scale} block and every ``normalized to unit variance'' step put the data.

\begin{figure}[t]
\centering\includegraphics[width=\textwidth]{figures/mechanism.pdf}
\caption{\textbf{The defect as a function of input scale.} \textbf{A}, Ratio of before-fix to after-fix ReHo against the SD of the time series handed to \code{3dReHo}, from a faithful port of the routine on Gaussian input (blue: white; green: band-passed 0.01--0.1\,Hz at TR 2\,s), 27-voxel neighbourhoods, 148 time points, 60 simulations per point. Orange: the whole-brain mean ratio for each of the 73 ds000030 participants at the SD of their own residuals. \textbf{B}, The same as relative error. Shaded band: percent-signal-change and z-scored data. Raw scanner units (SD $\gtrsim 5$) are unaffected.}
\label{fig:mech}
\end{figure}

\subsection{Per participant}
Across the 73 participants the residual SD handed to \code{3dReHo} averaged 0.35 (range 0.14--0.52). The before-fix build returned on average 0.27 of the after-fix whole-brain mean ReHo (0.130 versus 0.479), a relative error over all voxels of 75\% (range 61--92\%); it returned exactly zero in a mean 18\% of in-mask voxels (range 2--69\%) and about 0.30 of the correct value elsewhere. The error tracks residual amplitude across participants ($r = -0.88$). The before- and after-fix maps correlated $r = 0.59$ across voxels (range 0.31--0.76), the correlation rising with residual SD ($r = 0.64$; Fig.~\ref{fig:re}B). The defect reorders voxels rather than rescaling them, to a degree that varies with each participant's signal amplitude.

\subsection{Between groups}
Residual amplitude and censoring differed between the clinical groups (SCHZ residual SD $0.319 \pm 0.076$ versus CONTROL $0.374 \pm 0.051$; zeroed voxels 23\% versus 14\%; censored volumes 9.6\% versus 4.5\%), so the bias is group-dependent. Table~\ref{tab:group} and Figure~\ref{fig:re}C give the contrasts. The $t$-maps from the two builds correlated 0.63--0.64, disagreed in sign in 11--12\% of voxels, and their $p<0.001$ sets shared 7 of 104 voxels in the intersection mask and 15 of 293 in the 90\%-coverage mask; at $p<0.05$ the two builds agreed on about a third of their union (Jaccard 0.40 and 0.34). The whole-brain mean reduction of ReHo in schizophrenia, the simplest statistic such papers report, is significant under both builds ($t(71) = -3.60$, $p = 0.001$, $d = -0.85$ after the fix; $t = -3.46$, $p = 0.001$, $d = -0.81$ before it, with zeroed voxels counted as zero), with the effect a twentieth of a standard deviation larger under the fixed code; the maps behind the two statistics are, as the rest of this section shows, different maps.

\subsection{Cluster-level inference}
Table~\ref{tab:clust} reports the analysis a published design would: family-wise cluster correction at $\alpha = 0.05$. Both builds find corrected clusters, all in the direction SCHZ $<$ CONTROL, but not the same ones. At the conventional cluster-forming $p < 0.001$ the two builds' corrected results share no voxel in either mask: the code every paper ran reports reduced ReHo in precuneus, posterior cingulate and right fusiform cortex; the fixed code reports it in right middle temporal gyrus in the 90\%-coverage mask and finds nothing in the intersection mask. At $p < 0.01$ the midline posterior-cingulate/precuneus cluster is found by both builds (146 of 610 voxels shared in the intersection mask), while the supplementary-motor and right-lingual clusters exist only before the fix and the cerebellar-vermis cluster only after it. This is the concrete form of the exposure: at the sample size and thresholds the affected studies use, each pipeline yields a publishable corrected finding, and the findings are anatomically different. Neither pattern is a claim about schizophrenia; the contrast is unadjusted for the age and motion differences in Table~\ref{tab:demo}, and the point is the disagreement between two builds of one program on one set of inputs.

\begin{table}[t]
\centering\small
\caption{Cluster-level inference for SCHZ versus CONTROL (33 vs 40) under each build (\code{3dttest++ -Clustsim}, 10{,}000 sign-flip randomisations, NN1, bi-sided, $\alpha = 0.05$). All clusters are SCHZ $<$ CONTROL; labels are the Macro Labels atlas at the peak. Shared voxels: intersection / union of the two builds' corrected clusters.}
\label{tab:clust}
...
\begin{table}[t]
\centering\small
\caption{SCHZ versus CONTROL (33 vs 40, \code{3dttest++}), same participants and residuals, the only difference being which \code{3dReHo} produced the input maps.}
\label{tab:group}
\begin{tabular}{@{}lcc@{}}
\toprule
 & Intersection mask (6{,}488 vox) & 90\%-coverage mask (24{,}403 vox) \\
\midrule
Voxels $|t|>3.43$ ($p<.001$), before / after & 83 / 28 & 215 / 93 \\
\quad overlap (before $\cap$ after / before $\cup$ after) & 7 / 104 & 15 / 293 \\
Voxels $|t|>1.96$ ($p<.05$), before / after & 1{,}621 / 1{,}622 & 6{,}016 / 5{,}112 \\
\quad overlap (Jaccard) & 928 / 2{,}315 (0.40) & 2{,}807 / 8{,}321 (0.34) \\
Spatial correlation of $t$-maps & 0.64 & 0.63 \\
Voxels where $t$ changes sign & 11\% & 12\% \\
Mean $|t_{\mathrm{before}} - t_{\mathrm{after}}|$ & 0.64 & 0.67 \\
\bottomrule
\end{tabular}
\end{table}
