...
**How the input arises, and how often.** An exact 0 for every member present needs a ranked list that carries unexpressed genes at 0 (a DESeq2 or edgeR table with NA replaced by 0, a Seurat/Scanpy marker table exported with every gene) *and* a gene set none of whose present members is expressed. I checked how often that happens on real data: the airway RNA-seq experiment (GSE52778, all 63,677 Ensembl genes, DESeq2 statistics with NA set to 0: 13,757 of 39,609 symbol-mapped genes at 0) and the 10x PBMC 3k dataset (13,714 genes, 36 cell-type contrasts with up to 4,344 genes at exactly 0), each against every MSigDB 7.5.1 collection (about 23,000 sets with 15 to 500 members present). No set met the condition; the closest had 95 % of its members at 0. So with the standard collections this needs a small custom set of tissue- or cell-type-specific genes on a list that includes unexpressed genes; the defect is that when it does happen the program reports an ES, NES, p and FDR from a 0/0 hit weight with no warning, rather than refusing or falling back to equal weights.
