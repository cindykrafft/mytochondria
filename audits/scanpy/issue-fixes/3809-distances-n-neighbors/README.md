...
- This changes `.obsp["distances"]` for the pynndescent and custom-transformer paths (one fewer neighbour per row; `method="gauss"` connectivities on those paths are recomputed from the trimmed matrix and are unchanged in values, since `gauss` already truncated to `n_neighbors`). Anything downstream that consumed the extra neighbour (e.g. tools reading `obsp["distances"]` directly) will see the documented `n_neighbors - 1`.
