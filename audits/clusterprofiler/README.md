...
| **CP5** | **CONFIRMED on enrichit 0.1.4–0.1.5** (clusterProfiler 4.19.3–4.20.0 installed 2026-04 … 06); fixed in enrichit 0.2.0 (CRAN 2026-07-01) | comment on open #819 | ORA adjusted p over every gene set in the size window including `Count = 0` sets: GO BP, 198-gene query — 513 significant terms instead of 694 (BH over 6,379 instead of 3,856). |
...
1. Read the #819 thread and the maintainer's guide/book pages from a host that can reach them; post
   the CP5 comment only if the thread does not already point to enrichit ≥ 0.2.0.
