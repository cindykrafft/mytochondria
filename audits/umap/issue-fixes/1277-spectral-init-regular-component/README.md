...
| **#1224** "transform of a subset differs from transform of all then sub-selecting" (2025-10-20, 0 comments) | Not a wrong result. `alt-1224-transform-subset.out` (4 Gaussian blobs, 1,500 / 5,000 training points, 1,200 new, a 600-row subset): the bipartite graph rows and `init_graph_transform` are identical between the two batches on both the exact and the pynndescent path (`alt-1224-pynndescent-query.out`: `NNDescent.query` recall 0.95, 0 of 600 rows differ, single-row queries match both batches). On the exact path the embeddings differ by RMSE 0.58 / max 2.11, the same order as the same subset under another `transform_seed` (RMSE 0.67 / max 2.24), and the reporter's numbers (RMSE 0.42, max 2.44) match that noise floor. On the pynndescent path, where the batch maximum edge weight happened to coincide (0.4726 in both batches), subset and full transforms were bit-identical; on the exact path it differed (0.529 vs 0.465). The only batch-level quantities in `transform` are that maximum, used for the edge threshold `graph.data.max() / n_epochs` and for `make_epochs_per_sample`'s `weights / weights.max()` (the same normalisation `fit` applies to its whole graph), and the SGD's seeded negative-sampling stream. Making per-point results independent of the batch would mean a per-row normalisation, a design decision for the maintainers, not a fix. |
