...
rows=list(csv.DictReader(open(F+'subjects_summary_ext73.tsv'),delimiter='\t'))
ax=fig.add_subplot(top[1])
for grp,col,lab in [('CONTROL',BLUE,'Control (n = 40)'),('SCHZ',ORANGE,'Schizophrenia (n = 33)')]:
...
cax=fig.add_axes([0.35,0.045,0.3,0.011]); cb=fig.colorbar(im,cax=cax,orientation='horizontal',ticks=[-4,-2,0,2,4]); cb.set_label('t, schizophrenia vs control, 33 vs 40 (|t| > 1.96 shown)',color=INK2,fontsize=7); cb.ax.tick_params(labelsize=6.5,colors=INK2); cb.outline.set_edgecolor(RULE)
fig.text(0.02,0.475,'C',fontsize=12,fontweight='bold',color=INK)
fig.savefig(F+'figure1_73.png',dpi=300,bbox_inches='tight',facecolor='white'); fig.savefig(F+'figure1_73.pdf',bbox_inches='tight',facecolor='white'); fig.savefig(F+'figure1_73.tiff',dpi=300,bbox_inches='tight',facecolor='white',pil_kwargs={'compression':'tiff_lzw'})
print('saved')
