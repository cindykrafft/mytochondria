...
rows=list(csv.DictReader(open('../results/subjects_summary_AB.tsv'),delimiter='\t'))
...
for ax,(idx,ylabel,yr,real) in zip(axs,[(0,'ReHo before fix / after fix',(0,1.05),ratio_r),(1,'Relative error of ReHo before fix',(0,1.05),err_r)]):
    ...
    ax.scatter(sd_r,real,s=18,c=ORANGE,edgecolors='white',linewidths=0.6,zorder=4,label='ds000030 participants (n = 40), whole-brain mean')
