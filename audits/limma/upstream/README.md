...
author of the 4.0.0 C code that LM1's `devel` patch touches. Results-affecting changes go
...
- `man/arrayWeights.Rd` (the `method`, `tol`, `maxiter` items and the auto rule),
  ...
  `offset.prior` items and the Details paragraph that LM2's patch rewrites),
...
- edgeR's `voomLmFit` on 4.0.16 (apt) and 4.10.5 (the edgeR audit's `RELEASE_3_23` build)
  and `scaleOffset`'s help page, to state the offset convention LM2 relies on.
...
| file | what |
|---|---|
| `issue-lm1-arrayweights-reml-convergence.md` | the support-site post for LM1 (Title line + body), example run on 3.68.5 and devel |
| `0001-arrayWeights-fix-the-convergence-criterion-of-the-pr.patch` | LM1 fix (`src/awreml.c`) + test + changelog/NEWS against `devel` @ `57a8de72` (`git am`-clean) |
| `0001-arrayWeights-fix-the-convergence-criterion.RELEASE_3_23.patch` | LM1 fix (`R/arrayWeightsPrWtsREML.R`) + test + changelog against `RELEASE_3_23` @ `825d1c83` (CRLF branch: `git am --keep-cr`) |
| `issue-lm2-voom-offset-double-count.md` | the support-site post for LM2 |
| `0002-voom-give-an-edgeR-style-offset-precedence-over-lib.size.patch` | LM2 change (`R/voom.R`, `man/voom.Rd`) + test + changelog/NEWS against `devel`, on top of 0001 |
...
| id | tier | why |
|---|---|---|
| LM1 | **now, first** | wrong number on every version since 2019 at default settings of a documented method; no help-page text defends it; becomes the default sample-weight path of `voomLmFit()` in 4.0.0; one-line fix on each branch, a single post is the whole cost |
...
| N1, N2 | held (questions) | the df.prior floor of the unequal-df estimator; a help-page sentence for voom users on the `contrasts.fit` approximation — after LM1 has an answer |
...
1. Search the support site for "arrayWeights reml", "voomWithQualityWeights reml" and
   ".arrayWeightsPrWtsREML"; if nothing matches, post `issue-lm1-arrayweights-reml-convergence.md`
...
2. Record the post URL and every maintainer reply in `../README.md` and the top-level status
   table.
3. When LM1 is answered (or after a reasonable interval), search for "voom offset DGEList"
   and post `issue-lm2-voom-offset-double-count.md` with the `devel` patch. Nothing else goes
