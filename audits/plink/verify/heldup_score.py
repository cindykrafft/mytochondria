...
with tempfile.TemporaryDirectory() as tmp:
    ...
    cases = [
        ...
        ("no-mean-imputation sum (plink2 only; 1.9 rejects the pair)", ["no-mean-imputation"], False, False, False),
        ("center", ["center"], True, True, True),
        ("center no-mean-imputation (plink2 only; 1.9 rejects the pair)", ["center", "no-mean-imputation"], False, True, True),
    ]
    for label, mods, mi, ce, av in cases:
        if "plink2 only" in label:
            r, cnt = ref(mi, ce, av)
            cols = "maybefid,nallele,denom,dosagesum,scoreavgs,scoresums"
            run(PLINK2, ["--pedmap", pre, "--score", sf, "1", "2", "3", "cols=" + cols, *mods, "--out", os.path.join(tmp, "s2")])
            t2 = read_table(os.path.join(tmp, "s2.sscore")); idx2 = [int(s[1:]) - 1 for s in t2["IID"]]
            col = "SCORE1_AVG" if av else "SCORE1_SUM"
            d2 = max(abs(fnum(x) - r[i]) for i, x in zip(idx2, t2[col])); c2 = max(abs(fnum(x) - cnt[i]) for i, x in zip(idx2, t2["DENOM"]))
            print(f"{label:30s}: plink2 max|{col}-ref| {d2:.2e} |DENOM-ref| {c2:g}")
            continue
        r, cnt = ref(mi, ce, av)
        ...
        d19 = max(abs(fnum(x) - r[i]) for i, x in zip(idx, t["SCORESUM" if "SCORESUM" in t else "SCORE"]))
        # plink 1.9 documents CNT as the number of non-missing alleles regardless of imputation
        c19 = max(abs(fnum(x) - 2 * obs.sum(1)[i]) for i, x in zip(idx, t["CNT"]))
        ...
        print(f"{label:30s}: plink1.9 max|SCORE-ref| {d19:.2e} |CNT-2*nonmissing| {c19:g};  plink2 max|{col}-ref| {d2:.2e} |DENOM-ref| {c2:g}")
