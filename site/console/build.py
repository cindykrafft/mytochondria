...
r2("bcftools","samtools","bcftools","develop",None,
   ...
   order_note="Fork made and branch fix/mwu-biasz-int64 pushed 2026-09-23 (the kit patch rebased onto develop edf7fd9, one commit, your authorship, the Assisted-by trailer, no sign-off). Before opening the PR: fetch the branch, rewrite the commit message in your own words and git commit --amend -s, force-push, then the compare button. One filing only. The annotations were introduced in 1.13, so the cohort's two most-cited versions (1.9, 1.10.2) are unaffected; 1.13 onward is.")
...
ACTIONS=[
 ...
 dict(kind="pr", key="bcftools", id="BC1", why="the 64-bit fix with a regression BAM and test, on branch fix/mwu-biasz-int64 of your fork since 2026-09-23; the samtools AI policy applies: fetch the branch, rewrite the commit message yourself and add your own sign-off (git commit --amend -s), force-push, then open the PR"),
