...
r2("bcftools","samtools","bcftools","develop",None,
   ...
   order_note="Branch fix/mwu-biasz-int64 on your fork (2026-09-23): one commit 83b7888 on develop edf7fd9, your authorship, the Assisted-by trailer and your Signed-off-by (added at your instruction). Ready to file: the issue card, then the PR card; reword the commit message and PR body in your own words if you want to, the policy asks for that. One filing only. The annotations were introduced in 1.13, so the cohort's two most-cited versions (1.9, 1.10.2) are unaffected; 1.13 onward is.")
...
ACTIONS=[
 ...
 dict(kind="pr", key="bcftools", id="BC1", why="the 64-bit fix with a regression BAM and test, on branch fix/mwu-biasz-int64 of your fork (commit 83b7888, signed off in your name at your instruction); the samtools AI policy asks for a human-written commit message and PR body, so reword them if you want to; the PR body ends with the Assisted-by line as the policy requires"),
