...
ACTIONS=[
 ...
 dict(kind="issue", key="gsea", id="GS3", why="round 4's strongest finding, on a package 720 papers name (69 of them identify this desktop program, the rest run fgsea or clusterProfiler or do not say) and under the default scoring scheme: a gene set whose present members all score 0 gets a 0/0 hit weight, every running-sum comparison is false, and the set is reported with the running sum before its first hit as ES, plus NES, p and FDR of 0.000, no warning; 30 percent zeros in a 5000-gene list make a 40-gene all-zero set the second most negative result of the run; real-data check (airway bulk and PBMC 3k, every MSigDB collection): the precondition never occurred, so the text now leads with the silent 0/0 path and states the check"),
 dict(kind="pr", key="gsea", id="GS3", why="equal hit weights plus a warning, with a test that fails on master; fork GSEA-MSigDB/gsea-desktop first"),
 dict(kind="issue", key="bcftools", id="BC1", why="mpileup's default bias annotations (MQBZ, BQBZ, RPBZ, SCBZ, MQSBZ) accumulate their tie correction in a 32-bit int that wraps at 1291 reads in one quality bin; MAPQ is capped at 59 and histograms are pooled across samples, so 44 single-sample BAMs of 30 reads under the default -d 250 report MQBZ -7.0 for a true -36.7 and the score saturates near -11 whatever the real bias"),
